...
\begin{abstract}
We present a simply deployable interface that allows querying and downloading Holocene sea-level datapoints, formatted following the HOLSEA data template, hosted on a mySQL database. The interface uses AngularJS, is scalable to large datasets and can be deployed in few easy steps, that require only basic knowledge of SQL and HTML.
...
\begin{figure*}
	...
	\caption{Data filtering on the map. Only the data within the selection rectangle will be available on the data explorer interface.}
	\label{fig:1}
\end{figure*}
...
\begin{figure*}
	...
	\caption{Data filtering on the dat explorer interface.}
	\label{fig:2}
\end{figure*}
...
The age/elevation graph was built using a custom Scalable Vector Graphic (SVG) HTML element, extended with AngularJS directives to dynamically manage displayed data. The map and age/elevation graph are interconnected with the AngularJS framework, which allows asyncronous storage and rendering the interface elements base on this dynamic data. The entire interface is a one-page website, rendering different outputs based on data and user choices. 
